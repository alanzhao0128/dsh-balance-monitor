...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CpaPlanCard({ wide, t, refreshCpa, pollMs, warn, danger }) {
      ...
      // One row per group × window. CPA reports what is left; show what is used.
      const rows = [];
        ...
        children: [
          ...
          jsxs('div', {
            ...
            children: rows.map((item) => {
              ...
              const used = 100 - item.remaining;
              return jsxs('div', {
                key: item.key,
                style: { display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 },
                children: [
                  jsx('span', {
                    style: { flex: 'none', width: 52, fontSize: 11, lineHeight: '16px', color: 'var(--dsw-alias-label-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
                    title: t(`cpa.group.${item.group}`) || item.group,
                    children: `${groupShort} ${period}`,
                  }),
                  jsxs('div', {
                    style: { flex: 'none', width: 72, height: 4, borderRadius: 2, background: 'var(--dsw-alias-track-bg, rgba(128,128,128,0.25))', overflow: 'hidden' },
                    children: [
                      jsx('div', {
                        style: { height: '100%', width: used + '%', borderRadius: 2, background: arkColorOf(used, warn, danger), transition: 'width .3s', opacity: stale ? 0.55 : 1 },
                      }),
                    ],
                  }),
                  jsx('span', {
