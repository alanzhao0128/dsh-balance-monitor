...
window.__ModuleLoader__.load({
  id: '@alanzhao/dsh-balance-monitor',
  factory: (require) => {
    var module = { exports: {} };
    ...
    const zh = {
      ...
      // Google AI Pro（cliproxy / CPA 反代）
      'cpa.label': 'Google AI Pro',
      'cpa.short.gemini': 'Gemini',
      'cpa.short.claude_gpt': 'C/GPT',
      'cpa.group.gemini': 'Gemini 组',
      'cpa.group.claude_gpt': 'Claude/GPT 组',
      'cpa.error.unconfigured': '未配置 CPA 地址',
      'cpa.error.unauthorized': 'CPA 凭证未配置或被拒绝',
      'cpa.error.other': '读取 CPA 配额失败',
      'ark.reset': '已重置',
      ...
      'settings.cc.cpa': 'Google AI Pro（CPA 反代）',
      'settings.cc.cpa.basic': 'Basic 认证（user:pass）',
      'settings.cc.cpa.key': '管理密钥',
      'settings.cc.cpa.hint': '查 5h/周 剩余配额。Basic 认证与管理密钥由 CPA 服务器提供（nginx Basic 是第二把锁，CPA management key 是第三把锁）；插件据此读取 antigravity-priority 插件探测到的额度。',
      ...
      'settings.timeoutSecHint': '火山方舟 / Command Code / DeepSeek 官方用量请求超时',
      'settings.cpaBaseUrl': 'CPA 地址',
      'settings.cpaBaseUrlHint': '你自己的 CLIProxyAPI 管理地址，用于查 Google AI Pro 配额（留空则不查，例：https://cpa.example.com）',
      'settings.cpaStaleSec': 'CPA 快照过期阈值（秒）',
      'settings.cpaStaleSecHint': 'CPA 快照超过该时长即视为过期，插件自动补一次探测（默认 1200 秒；建议大于 CPA 侧定时探测间隔，如 15 分钟）',
    ...
    const en = {
      ...
      // Google AI Pro (cliproxy / CPA reverse proxy)
      'cpa.label': 'Google AI Pro',
      'cpa.short.gemini': 'Gemini',
      'cpa.short.claude_gpt': 'C/GPT',
      'cpa.group.gemini': 'Gemini',
      'cpa.group.claude_gpt': 'Claude/GPT',
      'cpa.error.unconfigured': 'CPA base URL not configured',
      'cpa.error.unauthorized': 'CPA credentials missing or rejected',
      'cpa.error.other': 'Failed to read CPA quota',
      'ark.reset': 'Reset',
      'cmd.label': 'Command Code',
      // settings page
      ...
      'settings.cc.cpa': 'Google AI Pro (CPA)',
      'settings.cc.cpa.basic': 'Basic auth (user:pass)',
      'settings.cc.cpa.key': 'Management key',
      'settings.cc.cpa.hint': 'Queries the 5h/weekly remaining quota. Both credentials come from your CPA server (nginx Basic is the second lock, the CPA management key the third); the plugin reads the quota probed by the antigravity-priority plugin.',
      ...
      'settings.timeoutSecHint': 'Volcano Ark / Command Code / DeepSeek official usage request timeout',
      'settings.cpaBaseUrl': 'CPA base URL',
      'settings.cpaBaseUrlHint': 'Your own CLIProxyAPI management address for the Google AI Pro quota (empty = not queried; e.g. https://cpa.example.com)',
      'settings.cpaStaleSec': 'CPA snapshot age limit (s)',
      'settings.cpaStaleSecHint': 'A CPA snapshot older than this counts as stale and the plugin requests a fresh probe automatically (default 1200 s; keep it above CPA-side scheduling, e.g. 15 min)',
    ...
    // ---- Google AI Pro card (cliproxy → CPA) -------------------------------

    /**
     * Quota for the reversed Google AI Pro (Antigravity) channel. The host RPC
     * returns one entry per model group (`gemini` / `claude_gpt`), each with a
     * 5h and a weekly window carrying the *remaining* share; the card renders
     * consumption (used %) so it reads like the Ark / Command Code cards.
     */
    function CpaPlanCard({ wide, t, refreshCpa, pollMs, warn, danger }) {
      const [state, setState] = useState(null);
      const [errorCode, setErrorCode] = useState(null);

      const tick = useCallback(async () => {
        try {
          const result = await refreshCpa();
          if (result && result.ok && result.value) {
            setState(result.value);
            setErrorCode(null);
          } else if (result && result.ok === false && result.error) {
            setErrorCode(result.error.code || 'other');
          }
        } catch {
          // keep the last known numbers
        }
      ...
      const stale = state ? state.stale === true : false;
      const plan = groups.find((g) => g.plan)?.plan ?? null;

      // One row per group × window, rendered with the same "used %" semantics
      // as the Ark / Command Code cards. CPA reports what is *left*, so the
      // bar value is `100 - remaining` — the number, the fill and the colour
      // then read exactly like the sibling cards.
      const rows = [];
      for (const g of groups) {
        for (const w of Array.isArray(g.windows) ? g.windows : []) {
          const remaining = Math.max(0, Math.min(100, w.remaining));
          rows.push({
            key: `${g.id}:${w.level}`,
            group: g.id,
            level: w.level,
      ...
      // Collapsed rail mirrors the Ark card: the 5h window of the active model
      // group, else any 5h window, else the most-used one.
      const activeGroup = state && typeof state.activeModelGroup === 'string' ? state.activeModelGroup : null;
      const railRow = rows.find((r) => r.level === '5h' && r.group === activeGroup)
        ?? rows.find((r) => r.level === '5h')
        ?? (rows.length > 0 ? rows.reduce((a, b) => (a.percentUsed > b.percentUsed ? a : b)) : null);

      if (!wide) {
        return jsx('div', {
          style: railStyle,
          title: railRow
            ? `${t('cpa.label')} · ${t(`ark.period.${railRow.level}`) || railRow.level} ${Math.round(railRow.percentUsed)}%`
            : (errorCode ? `${t('cpa.label')} · ${t(`cpa.error.${errorCode}`) || t('cpa.error.other')}` : t('cpa.label')),
          children: railRow ? `${Math.round(railRow.percentUsed)}%` : '—',
        });
      }

      // No rows yet: if the host reported why (today: an unconfigured base URL
      // or rejected credentials) say so instead of leaving an empty card, so a
      // missing `network.cpaBaseUrl` is self-explanatory in the sidebar.
      if (rows.length === 0 && errorCode) {
        return jsxs('div', {
          style: cardStyle,
          title: `${t('cpa.label')} · ${t(`cpa.error.${errorCode}`) || t('cpa.error.other')}`,
          children: [
            jsxs('div', {
              style: rowStyle,
              children: [
                jsx('span', { style: { ...labelStyle, overflow: 'hidden', textOverflow: 'ellipsis' }, children: t('cpa.label') }),
                jsx('strong', { style: valueStyle(false), children: '—' }),
              ],
            }),
            jsx('div', {
              ...
              children: t(`cpa.error.${errorCode}`) || t('cpa.error.other'),
            }),
          ],
        });
      }
      ...
      return jsxs('div', {
        style: cardStyle,
        title: plan ? `${t('cpa.label')} · ${plan.toUpperCase()}` : t('cpa.label'),
        children: [
          jsxs('div', {
            style: rowStyle,
            children: [
              jsx('span', {
                style: labelStyle,
                children: plan ? `${t('cpa.label')} · ${plan.toUpperCase()}` : t('cpa.label'),
              }),
            ],
          }),
          jsxs('div', {
            style: { display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0 },
            children: rows.map((item) => {
              const groupShort = t(`cpa.short.${item.group}`) || item.group;
              const period = t(`ark.period.${item.level}`) || item.level;
              return jsxs('div', {
                key: item.key,
                style: { display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 },
                children: [
                  jsx('span', {
                    style: { flex: 'none', width: 64, fontSize: 11, lineHeight: '16px', color: 'var(--dsw-alias-label-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
                    title: t(`cpa.group.${item.group}`) || item.group,
                    children: `${groupShort} ${period}`,
                  }),
                  jsxs('div', {
                    style: { flex: 'none', width: 62, height: 4, borderRadius: 2, background: 'var(--dsw-alias-track-bg, rgba(128,128,128,0.25))', overflow: 'hidden' },
                    children: [
                      jsx('div', {
                        style: { height: '100%', width: item.percentUsed + '%', borderRadius: 2, background: arkColorOf(item.percentUsed, warn, danger), transition: 'width .3s', opacity: stale ? 0.55 : 1 },
                      }),
                    ],
                  }),
                  jsx('span', {
    ...
    const FIELDS = [
      ...
      { path: ['network', 'cpaBaseUrl'], type: 'text', group: 'network', labelKey: 'settings.cpaBaseUrl', hintKey: 'settings.cpaBaseUrlHint' },
      { path: ['network', 'cpaStaleMs'], type: 'secMs', group: 'network', labelKey: 'settings.cpaStaleSec', hintKey: 'settings.cpaStaleSecHint' },
      // ---- 凭证 ----
    ...
    function SettingsPage({ t, connection, scope, remote }) {
      ...
      useEffect(() => {
        ...
        (async () => {
          try {
            const result = await connection.rpc.call('/api', 'credential-status/snapshot', {});
      ...
      const save = async () => {
        ...
        try {
          ...
          // dsh-settings >= 0.1.2: the bound scope owns namespace writes
          // (scope.mutate); connection.api.settings.mutate was removed.
      ...
      const saveCredential = async (ref) => {
        ...
        try {
          // dsh 0.1.2 removed connection.api.credentials.set; credential
          // writes go through ctx.remote.credentials.set(ref, value).
          if (!remote || typeof remote.credentials?.set !== 'function') {
            throw new Error('credentials remote unavailable');
          }
      ...
      // Google AI Pro block (cliproxy → CPA): two writable credentials.
      const cpaBlock = jsxs('div', { key: 'cpa', style: { marginBottom: 14, borderTop: '1px solid var(--dsw-alias-border-l2)', paddingTop: 10 }, children: [
        jsx('div', { style: { fontSize: 12.5, fontWeight: 600, lineHeight: '18px', color: 'var(--dsw-alias-label-secondary)', marginBottom: 6 }, children: tr('settings.cc.cpa') }),
        credInputRow('settings.cc.cpa.basic', 'CPA_BASIC_AUTH', null),
        credInputRow('settings.cc.cpa.key', 'CPA_MANAGEMENT_KEY', null),
        jsx('div', { style: { fontSize: 11, lineHeight: '16px', color: 'var(--dsw-alias-label-tertiary)', margin: '2px 0 4px' }, children: tr('settings.cc.cpa.hint') }),
      ]});
     ...
     * dsh 0.1.2 removed connection.api.sessions.models; the provider fact now
     * comes from the host /session-provider RPC (agentDefaultModel, which DSH
     * keeps in sync with session model switches via saveSelection).
     */
    function ChannelCard({ wide, t, refresh, refreshArk, refreshCmdCode, refreshCpa, sessions, connection, remote, queryProvider, scope, cfg }) {
      ...
      useEffect(() => {
        ...
        const check = async () => {
          ...
          try {
            ...
            const name = value.name || provider;
      ...
      if (CHANNELS[state.provider]) {
        ...
        if (ch.type === 'cpa-plan') return jsx(CpaPlanCard, { wide, t, refreshCpa, pollMs, warn, danger });
      }
      return jsx(UnsupportedCard, { wide, t, name: state.name });
    }

    const inject = ['connection', 'slots', 'locale', 'sessions', 'remote', 'remote.credentials', 'settingsScope'];
    ...
    function apply(ctx) {
      ...
      const refresh = () => connection.rpc.call('/api', 'balance/snapshot', {});
      const refreshArk = () => connection.rpc.call('/api', 'ark-quota/snapshot', {});
      const refreshCmdCode = () => connection.rpc.call('/api', 'cmdcode-quota/snapshot', {});
      const refreshCpa = () => connection.rpc.call('/api', 'cpa-quota/snapshot', {});
      // dsh 0.1.2 removed connection.api.sessions.models; the current model
      // channel is resolved host-side per session via our own RPC (pass the
      // active session id so the card follows whichever session is selected).
      const queryProvider = async (sessionId) => {
        const result = await connection.rpc.call('/api', 'session-provider/snapshot', { sessionId });
      ...
      ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
        {
          ...
          id: 'balance-monitor',
          locale: NS,
          inject: () => ({ refresh, refreshArk, refreshCmdCode, refreshCpa, sessions, connection, remote, queryProvider, scope, cfg }),
        },
        ChannelCard,
      ));
