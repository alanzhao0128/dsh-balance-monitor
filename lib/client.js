...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CmdCodeCard({ wide, t, refreshCmdCode }) {
      const [state, setState] = useState(null);
      ...
      useEffect(() => {
        tick();
        const timer = window.setInterval(tick, CMD_POLL_MS);
        const onVisible = () => {
          if (document.visibilityState === 'visible') tick();
        };
        document.addEventListener('visibilitychange', onVisible);
     ...
     */
    function ChannelCard({ wide, t, refresh, refreshArk, refreshCmdCode, sessions, connection, remote, queryProvider }) {
      ...
      if (CHANNELS[state.provider]) {
        ...
        if (ch.type === 'cmdcode-plan') return jsx(CmdCodeCard, { wide, t, refreshCmdCode });
      }
      ...
      ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
        {
          ...
          id: 'balance-monitor',
          locale: NS,
          inject: () => ({ refresh, refreshArk, refreshCmdCode, sessions, connection, remote, queryProvider }),
        },
        ChannelCard,
      ));
    }
