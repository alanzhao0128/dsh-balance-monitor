...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Channel registry: provider id (as registered by the host LLM adapter,
     * e.g. `deepseek-official` from llm-deepseek) → card behavior.
     * A provider not present here renders the "unsupported channel" placeholder.
     * Future channels (opencode-go, dashscope, …) get a row here.
     */
    const CHANNELS = {
      'deepseek-official': 'balance', // DeepSeek 官方 → 余额/花费卡片
    };
     ...
     */
    function ChannelCard({ wide, t, refresh, sessions, connection, remote, queryProvider }) {
      ...
      if (CHANNELS[state.provider]) {
        return jsx(BalanceCard, { wide, t, refresh });
      }
      ...
      ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
        {
          ...
          id: 'balance-monitor',
          locale: NS,
          inject: () => ({ refresh, sessions, connection, remote, queryProvider }),
        },
        ChannelCard,
      ));
    }
