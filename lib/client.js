...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Reset countdown: epoch-ms → `2d 3h 15m` (days/hours omitted when zero). */
    const fmtReset = (ts) => {
      if (!ts) return '—';
      ...
      if (!wide) {
        return jsx(
          'div',
          {
            style: railStyle,
            title: worst
              ? `${t('ark.label')} · ${worst.level} ${Math.round(worst.percentRemaining)}%`
              : t('ark.label'),
            children: worst ? `${Math.round(worst.percentRemaining)}%` : '—',
          },
        );
      }
            ...
            children: rows.map((item) => {
              ...
              return jsxs('div', {
                ...
                children: [
                  ...
                  jsxs('div', {
                    style: { flex: 'none', width: 56, height: 4, borderRadius: 2, background: 'var(--dsw-alias-track-bg, rgba(128,128,128,0.25))', overflow: 'hidden' },
                    children: [
                      jsx('div', {
                  ...
                  jsx('span', {
                    ...
                    children: Math.round(item.percentRemaining) + '%',
                  }),
                  jsx('span', {
                    style: { flex: 'none', fontSize: 10, lineHeight: '16px', color: 'var(--dsw-alias-label-tertiary)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' },
