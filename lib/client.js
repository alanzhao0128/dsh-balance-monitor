...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'settings.cpaBaseUrlHint': 'CLIProxyAPI 管理地址，用于查 Google AI Pro 配额（默认 https://cpa.alanzhao.xyz）',
    ...
    const en = {
      ...
      'settings.cpaBaseUrlHint': 'CLIProxyAPI management address for the Google AI Pro quota (default https://cpa.alanzhao.xyz)',
    ...
    /**
     * Colour for a *remaining* share: a nearly full bar means plenty left, so
     * the thresholds are mirrored relative to {@link arkColorOf} (which grades
     * usage). `remaining <= 100 - danger` is the danger zone.
     */
    const cpaColorOf = (remaining, warn, danger) => {
      if (remaining <= 100 - danger) return '#e5484d';
      if (remaining <= 100 - warn) return '#f5a524';
      return '#46a758';
    };
    ...
    function CpaPlanCard({ wide, t, refreshCpa, pollMs, warn, danger }) {
      ...
      // One row per group × window. Unlike the Ark / Command Code cards these
      // are *remaining* quota windows, so the number, the bar fill and the
      // colour all express "how much is left" (full + green = plenty).
      const rows = [];
      for (const g of groups) {
        for (const w of Array.isArray(g.windows) ? g.windows : []) {
          rows.push({
            ...
            group: g.id,
            level: w.level,
            remaining: Math.max(0, Math.min(100, w.remaining)),
            resetAt: w.resetAt,
          });
        }
      }
      // Collapsed rail: the 5h window (the near-term quota users watch most).
      const railRow = rows.find((r) => r.level === '5h')
        ?? (rows.length > 0 ? rows.reduce((a, b) => (a.remaining < b.remaining ? a : b)) : null);

      if (!wide) {
        return jsx('div', {
          style: railStyle,
          title: railRow
            ? `${t('cpa.label')} · ${t(`ark.period.${railRow.level}`) || railRow.level} ${Math.round(railRow.remaining)}%`
            : t('cpa.label'),
          children: railRow ? `${Math.round(railRow.remaining)}%` : '—',
        });
      }
              ...
              return jsxs('div', {
                ...
                children: [
                  ...
                  jsxs('div', {
                    ...
                    children: [
                      jsx('div', {
                        style: { height: '100%', width: item.remaining + '%', borderRadius: 2, background: cpaColorOf(item.remaining, warn, danger), transition: 'width .3s', opacity: stale ? 0.55 : 1 },
                      }),
                    ],
                  }),
                  jsx('span', {
                    style: { flex: 'none', minWidth: 34, textAlign: 'right', fontSize: 11, lineHeight: '16px', fontVariantNumeric: 'tabular-nums', color: 'var(--dsw-alias-label-primary)', opacity: stale ? 0.55 : 1 },
                    children: `${Math.round(item.remaining)}%`,
                  }),
                  jsx('span', {
                    style: { flex: 'none', marginLeft: 'auto', fontSize: 10, lineHeight: '16px', color: 'var(--dsw-alias-label-tertiary)', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' },
                    children: fmtReset(item.resetAt),
