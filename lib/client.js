...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      const renderGroup = (group) => {
        ...
        const children = fields.map((spec) => {
          ...
          if (spec.type === 'channel-toggle') {
            const enabled = Array.isArray(fieldValue(spec)) ? fieldValue(spec) : [];
            return jsx('div', {
                    ...
                    children: [
                      jsx('input', {
                        ...
                        onChange: (e) => {
                          ...
                          onChange(pathKey(spec.path), next);
                        },
    ...
    function ChannelCard({ wide, t, refresh, refreshArk, refreshCmdCode, scope, cfg }) {
      ...
      const pids = Array.isArray(enabled) ? enabled.filter((pid) => CHANNELS[pid]) : [];
