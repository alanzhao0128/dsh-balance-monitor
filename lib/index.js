...
/** How long the host caches an upstream GetAFPUsage response (matches the 60s balance cadence). */
const ARK_CACHE_MS = 60000  // 1 minute
...
async function readArkCredentials() {
  const ak = process.env.ARK_ACCESS_KEY_ID
    ?? (await (async () => { try { const y = await readFile(join(dshHome(), CREDENTIALS_FILE), 'utf8'); return y.match(/^ARK_ACCESS_KEY_ID:\s*(\S+)/m)?.[1]; } catch { return null; } })());
  const sk = process.env.ARK_SECRET_ACCESS_KEY
    ?? (await (async () => { try { const y = await readFile(join(dshHome(), CREDENTIALS_FILE), 'utf8'); return y.match(/^ARK_SECRET_ACCESS_KEY:\s*(\S+)/m)?.[1]; } catch { return null; } })());
  return { ak, sk };
}
