...
async function fetchCpaQuota() {
  ...
  const snapshotPath = '/v0/management/plugins/antigravity-priority/snapshot/latest'
  let res = await cpaRequest(base, snapshotPath, basicAuth, managementKey)
  ...
  if (groups.length === 0) {
    // Cold plugin (typically right after a CPA restart): ask for one probe.
    try {
      await cpaRequest(base, '/v0/management/plugins/antigravity-priority/run?mode=probe', basicAuth, managementKey, 'POST')
      res = await cpaRequest(base, snapshotPath, basicAuth, managementKey)
      if (res.status === 200 && res.body) {
