...
//
// Today's spend ("今日花费") has two sources, in order of preference:
//
...
//    API (platform.deepseek.com/api/v0/usage/cost — the same date-filterable
//    data the platform web console shows) and picks today's row. Source is
//    reported as "official" in the snapshot.
// 2. Estimate (fallback): without the platform token, the host meters the
//    balance — day-start baseline kept in the state file, spend accumulating
//    balance *drops* only (a refill raises the balance and is not
//    consumption, so it never inflates spend nor washes out spent amounts).
//    Source is reported as "estimate".
//
