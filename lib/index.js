...
export function apply(ctx, config) {
  ...
  // The card follows the DSH default/effective model channel. dsh 0.1.2 removed
  // the browser connection.api.sessions.models aggregate (its `current` read);
  // the equivalent host-side fact is ctx.agentDefaultModel.currentSelection()
  // — DSH keeps the default in sync when a session switches model (selectModel
  // calls agentDefaultModel.saveSelection), so this tracks session switches
  // the same way the old `.current` did. Returns { provider, model } or a null
  // value when the service is absent (no session/channel to show).
  ctx.connection.rpc.handle(
    '/session-provider',
    async (_endpoint, _payload, _signal) => {
      refreshServices()
