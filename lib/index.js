...
/** How long the host caches upstream Command Code usage responses. */
const CMDCODE_CACHE_MS = 40000  // 40s < 60s client poll
const CMDCODE_UPSTREAM_TIMEOUT_MS = 20000
...
/** Read the Command Code API key: env first, then .credentials.yaml (indented-safe). */
async function readCommandCodeKey() {
  if (process.env.COMMANDCODE_API_KEY) return process.env.COMMANDCODE_API_KEY
  try {
    const yaml = await readFile(join(dshHome(), CREDENTIALS_FILE), 'utf8')
    return yaml.match(/^\s*COMMANDCODE_API_KEY:\s*(\S+)/m)?.[1] ?? null
  } catch {
    return null
  }
}
...
function parseCommandCodeQuota(credits, summary, subscription) {
  ...
  if (monthlyCap > 0) {
    windows.push({
      ...
      resetAt: Number(subscription?.data?.currentPeriodEnd)
        ? new Date(subscription.data.currentPeriodEnd).getTime()
        : null,
    })
  }
  return windows
...
async function fetchCommandCodeQuota() {
  ...
  const getJson = async (path) => {
    const res = await fetch(`${CMDCODE_API_BASE}${path}`, {
      headers,
      signal: AbortSignal.timeout(CMDCODE_UPSTREAM_TIMEOUT_MS),
    })
...
export function apply(ctx, config) {
  ...
  ctx.connection.rpc.handle(
    ...
    async (_endpoint, _payload, _signal) => {
      try {
        const now = Date.now();
        if (cmdcodeCache !== null && now - cmdcodeCache.at < CMDCODE_CACHE_MS) {
