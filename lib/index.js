  ...
  ctx.connection.rpc.handle(
    ...
    async (endpoint, _payload, signal) => {
      try {
        ...
        const date = today()
        const dayStart = state.date === date ? state.dayStart : balance.total
        if (state.date !== date) {
          await saveState({ date, dayStart, lastTotal: balance.total, lastCurrency: balance.currency, updatedAt: Date.now() })
        }
        const snapshot = {
          date,
          dayStart,
          ...
          spent: Math.max(0, dayStart - balance.total),
          updatedAt: Date.now(),
          stale: false,
        }
        last = snapshot
        ...
        if (fallback && typeof fallback.lastTotal === 'number') {
          return {
            ok: true,
            value: {
              date: fallback.date,
              dayStart: fallback.dayStart,
              total: fallback.lastTotal,
              currency: fallback.lastCurrency ?? 'CNY',
              available: false,
              spent: Math.max(0, fallback.dayStart - fallback.lastTotal),
